...
```bash
go build -o bifrost ./client/cmd/bifrost
```

The client and protocol are one Go module; release builds can be produced from
the repository root without a workspace file.
...
The canonical wire contract lives in the `protocol/` directory:
...
See [client/docs/MODEL.md](client/docs/MODEL.md) for the data and safety contracts and
[client/docs/OPERATIONS.md](client/docs/OPERATIONS.md) for backup, cleanup, and recovery
guidance.
...
Release steps and compatibility rules are documented in
